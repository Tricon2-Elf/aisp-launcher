...
// ffmpeg, stream:<url> lets ffmpeg open the URL itself. ffmpeg letterboxes into the video box at a constant rate and writes one AVI
...
// bundled streamlink\ffmpeg\ffmpeg.exe); AISP_STREAMLINK and AISP_FFMPEG override the paths.
#include "source.h"
...
// ffmpeg re-requests an HTTP input from the current byte on a new connection when the server
// drops it, instead of failing.
...
{
    wchar_t ffmpeg[MAX_PATH] = {}, streamlink[MAX_PATH] = {};
    ...
    wchar_t input[2048] = {};
    wchar_t inputArgs[2300]; // "-i ..." with the options the input needs
    inputArgs[0] = L'\0';
    ...
    else if (_wcsnicmp(stream->source, L"stream:", 7) == 0)
    {
        ...
        if (_wcsnicmp(input, L"http", 4) == 0)
            StringCchPrintfW(inputArgs, 2300, L"%s-i \"%s\"", kHttpReconnect, input);
    }
    else
    {
        SetStatus(stream, L"ffmpeg source: expected streamlink:<url> or stream:<url>");
        return 0;
    }
    if (!inputArgs[0])
        StringCchPrintfW(inputArgs, 2300, L"-i \"%s\"", input);
    ...
    if (stream->audioWanted)
        StringCchPrintfW(audioOutput, 256, L" -map 0:a:0? -af \"aresample=async=1000:first_pts=0\" -ac %u -ar %u -c:a pcm_f32le", stream->channels, stream->sampleRate);
